...
import { CreditCard, Lock, CheckCircle2, ArrowLeft, Gift, Zap } from 'lucide-react'
...
function CheckoutForm() {
  ...
  useEffect(() => {
    if (bundleParam && bundles.length > 0) {
      const matchedBundle = bundles.find(bundle => bundle.id === bundleParam)
      if (matchedBundle) {
  ...
  const totalPrice = selectedBundleData ? selectedBundleData.totalPrice : planDetails.price
  const savingsText = selectedBundleData ? `Save ${selectedBundleData.discount}%` : undefined
      ...
      <div className="max-w-md mx-auto mt-12">
        <div className="glass-card p-8 text-center space-y-4 border-emerald-500/30 ring-1 ring-emerald-500/50">
          ...
          <p className="text-slate-400">
            You have successfully subscribed to {selectedBundleData ? `the ${selectedBundleData.name}` : `the ${planName} plan`}.
          </p>
    ...
    <div className="max-w-5xl mx-auto">
      ...
      <div className="space-y-8">
        ...
        {!loadingBundles && bundles.length > 0 && (
          <div>
            <h2 className="text-lg font-semibold text-white mb-4">Special Bundle Offers</h2>
                ...
                <button
                  ...
                  className={`p-4 rounded-lg border-2 transition-all ${
                    selectedBundle === bundle.id
                      ...
                      : 'border-slate-600/50 bg-slate-900/30 hover:border-blue-500/50'
                  }`}
                >
                  <div className="flex items-start justify-between">
                    ...
                    <div className="text-right">
                      <div className="text-lg font-bold text-white">${bundle.totalPrice.toFixed(2)}</div>
            ...
            <div className="glass-card p-6">
              ...
              <div className="space-y-4 mb-6">
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400">{selectedBundleData ? 'Bundle' : 'Plan'}</span>
                  <span className="font-medium text-white flex items-center gap-2">
                    {selectedBundleData ? selectedBundleData.name : planName}
                    {savingsText && <span className="text-xs bg-emerald-500/20 text-emerald-300 px-2 py-0.5 rounded">{savingsText}</span>}
                  </span>
                </div>
                <div className="flex justify-between text-sm">
